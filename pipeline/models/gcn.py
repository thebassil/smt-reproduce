"""GCNCard -- Graph Convolutional Network model card."""
...
try:
    ...
    raise ImportError(
        "GCNCard requires torch and torch_geometric. "
        "Install with: pip install torch torch_geometric"
    ) from e


class _GCNModel(nn.Module):
    """GCN with mean pooling and MLP head."""
...
class GCNCard(ModelCard):
    """GCN-based algorithm selection model for graph-structured features."""

    canvas_id: ClassVar[str] = "ext_model_gcn"
    ...
    def __init__(
        ...
        self.model_: Optional[_GCNModel] = None
        self.config_names_: List[str] = []
        self.in_dim_: int = 0

    def _build_model(self, in_dim: int, out_dim: int) -> _GCNModel:
        return _GCNModel(
            in_dim=in_dim,
