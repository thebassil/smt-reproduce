"""GINCard -- Graph Isomorphism Network model card."""
...
try:
    ...
    raise ImportError(
        "GINCard requires torch and torch_geometric. "
        "Install with: pip install torch torch_geometric"
    ) from e


class _GINModel(nn.Module):
    """Graph Isomorphism Network with sum pooling and MLP head."""
...
class GINCard(ModelCard):
    """GIN-based algorithm selection model for graph-structured features."""

    canvas_id: ClassVar[str] = "ext_model_gin"
    ...
    def __init__(
        ...
        self.model_: Optional[_GINModel] = None
        self.config_names_: List[str] = []
        self.in_dim_: int = 0

    def _build_model(self, in_dim: int, out_dim: int) -> _GINModel:
        return _GINModel(
            in_dim=in_dim,
    ...
    def fit(
        ...
        for epoch in range(self.epochs):
            ...
            for batch in loader:
                ...
                # batch.y may be stacked; reshape to (batch_size, n_configs)
