"""GraphSAGECard -- GraphSAGE model card."""
...
try:
    ...
    raise ImportError(
        "GraphSAGECard requires torch and torch_geometric. "
        "Install with: pip install torch torch_geometric"
    ) from e


class _GraphSAGEModel(nn.Module):
    """GraphSAGE with mean pooling and MLP head."""
    ...
    def __init__(
        ...
        for i in range(n_layers):
            ...
            self.convs.append(SAGEConv(dim_in, hidden_dim))
...
class GraphSAGECard(ModelCard):
    """GraphSAGE-based algorithm selection model for graph-structured features."""

    canvas_id: ClassVar[str] = "ext_model_graphsage"
    ...
    def __init__(
        ...
        self.model_: Optional[_GraphSAGEModel] = None
        self.config_names_: List[str] = []
        self.in_dim_: int = 0

    def _build_model(self, in_dim: int, out_dim: int) -> _GraphSAGEModel:
