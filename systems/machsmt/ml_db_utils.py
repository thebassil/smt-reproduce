...
Used by machsmt_portfolio_trainer.py and machsmt_evaluate_decisions.py.
...
def write_decisions_to_db(db_path, selector_id, decisions, instance_map,
    ...
    Args:
        decisions: list of dicts with keys:
            benchmark (file_path), predicted_solver_config ("solver::config"),
    ...
    for d in decisions:
        ...
        if instance_id is None:
            print(f"  WARN: no instance_id for {d['benchmark']}, skipping")
            skipped += 1
            continue

        config_id = solver_config_map.get(d['predicted_solver_config'])
        if config_id is None:
            print(f"  WARN: no config_id for {d['predicted_solver_config']}, skipping")
            skipped += 1
            continue
...
def load_decisions_from_db(db_path, selector_name):
    ...
    Returns {file_path: "solver::config_name"} dict,
    matching the format of load_machsmt_decisions() CSV loader.
    """
    conn = sqlite3.connect(db_path, timeout=30)
    rows = conn.execute(
        """SELECT i.file_path, c.solver || '::' || c.name
           FROM decisions d
    ...
    conn.close()
    return {file_path: solver_config for file_path, solver_config in rows}
