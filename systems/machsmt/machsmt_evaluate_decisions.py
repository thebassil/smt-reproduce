...
def main():
    ...
    parser.add_argument(
        "--selector-name",
        default=None,
        help="Load decisions from DB for this ML selector name (falls back to CSV)"
    )
    ...
    # Load MachSMT decisions: DB first (if --selector-name), then CSV fallback
    machsmt_decisions = None
    if args.selector_name:
        try:
            from ml_db_utils import load_decisions_from_db
            print(f"\nLoading decisions from DB for selector '{args.selector_name}'...")
            machsmt_decisions = load_decisions_from_db(args.db, args.selector_name)
            if machsmt_decisions:
                print(f"Loaded {len(machsmt_decisions)} decisions from DB")
            else:
                print(f"No decisions found in DB for '{args.selector_name}', falling back to CSV")
                machsmt_decisions = None
        except Exception as e:
            print(f"DB load failed ({e}), falling back to CSV")

    if machsmt_decisions is None and os.path.exists(args.decisions_csv):
    ...
    elif machsmt_decisions is None:
